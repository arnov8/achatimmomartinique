...
export default function Home() {
  ...
  const getEcartPrixM2 = (annonce: AnnonceRaw) => {
    ...
    return { prixM2, ecart: ((prixM2 - 3500) / 3500) * 100 };
  };
  ...
  const simulation = useMemo(() => {
    ...
    const mensualite = (capital * tauxMensuel) / (1 - Math.pow(1 + tauxMensuel, -(25 * 12)));
    return { mensualite: Math.round(mensualite) };
  ...
  const rendementData = useMemo(() => {
    ...
    return { brut, net: brut * 0.8, loyerEstime };
  }, [investAnnonce]);
    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="max-w-[1600px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          <div className="space-y-6">
            ...
            <p className="text-lg text-slate-500 font-medium max-w-xl">
              Centralisation intelligente pour votre futur chez-vous.
            </p>
      ...
      <section id="listing" className="max-w-[1600px] mx-auto px-4 md:px-6 -mt-10 relative z-10">
        <div className="bg-white p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] border border-slate-100">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            ...
            <FilterBox label="Surface min" onChange={setFilterSurface}>
              ...
              {[20, 40, 60, 80, 100].map(s => <option key={s} value={s}>{s} m² +</option>)}
            </FilterBox>
            ...
            <FilterBox label="Prix Max" onChange={setFilterPrixMax}>
              ...
              {[300000, 500000, 750000].map(p => <option key={p} value={p}>{p.toLocaleString()} €</option>)}
            </FilterBox>
            <div className="flex flex-col gap-3 justify-end">
              <button onClick={() => { setFilterCommune(""); setFilterType(""); }} className="w-full bg-slate-100 text-slate-600 font-black uppercase text-[10px] py-4 rounded-xl">
                Reset
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-12 mb-8 gap-6">
          <div className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-3">
            <span className="text-sm font-bold text-slate-800">{filteredAnnonces.length} annonces</span>
          </div>
          <button 
            onClick={() => setInvestorMode(!investorMode)}
            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${investorMode ? 'bg-indigo-600 border-indigo-600 text-white shadow-lg' : 'bg-white border-slate-200 text-slate-600'}`}
          >
      ...
      {selectedAnnonce && simulation && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl">
            <h3 className="text-xl font-black mb-6 uppercase">Estimer mon crédit</h3>
            <input type="number" value={apport} onChange={(e) => setApport(parseInt(e.target.value))} className="w-full bg-slate-50 border p-4 rounded-xl mb-4 font-bold" />
      ...
      {investAnnonce && rendementData && (
        <div className="fixed inset-0 bg-indigo-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 shadow-2xl">
            <h3 className="text-xl font-black mb-6 uppercase">Analyse Investisseur</h3>
            <div className="bg-indigo-50 p-6 rounded-2xl flex justify-around mb-8">
              <div><p className="text-2xl font-black text-indigo-600">{rendementData.brut.toFixed(2)} %</p><p className="text-[9px] font-bold uppercase opacity-50">Brut</p></div>
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative group">
        <select onChange={(e) => onChange(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl px-4 py-3.5 text-sm font-bold text-slate-800 outline-none appearance-none cursor-pointer shadow-sm">
          {children}
        </select>
